...
const PortfolioCarouselView = (
    {
        activePortfolios,
    }: {
        activePortfolios: Resource[];
    }) => {
    const maxScrollWidth = useRef<number>(0);
    const [currentIndex, setCurrentIndex] = useState<number>(0);
    const [itemsPerPage, setItemsPerPage] = useState<number>(3); // Default to 3
    const carousel = useRef<HTMLDivElement | null>(null);
    const totalPages = Math.ceil(activePortfolios?.length / itemsPerPage);
    const [showConfirmation, setShowConfirmation] = useState<boolean>(false);
    const [selectedId, setSelectedId] = useState<number | null>(null); // State to hold the ID of the item to be deleted
    ...
    const handleDelete = () => {
        if (selectedId !== null) {
            ...
            {
                onSuccess: () => {
                    if (activePortfolios?.length % itemsPerPage === 1 && currentIndex > 0) {
        ...
        <div className="carousel">
            ...
            <div className="relative overflow-hidden">
                <div className="relative z-0 flex items-center gap-8 overflow-hidden lg:justify-center justify-left scroll-smooth snap-x snap-mandatory touch-pan-x" ref={carousel}>
                    {activePortfolios?.slice(currentIndex * itemsPerPage, (currentIndex + 1) * itemsPerPage).map((resource: Resource, index: number) => (
                        <div className="relative flex flex-col gap-2" key={index}>
                            <div className="relative text-center bg-gray-400 h-36 w-50 rounded-2xl sm:h-52 sm:w-72 carousel-item snap-start">
                                <a
                                    href={resource?.link}
...
export default PortfolioCarouselView;
